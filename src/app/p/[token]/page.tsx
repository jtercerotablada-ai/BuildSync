 ...
 *
 * Everything rendered here comes from buildClientProjectView and nothing
 * else — this file must never reach for Prisma itself, or the "one place
 * decides what a client sees" guarantee stops being true.
 */

// Always render fresh: a revoked link must stop working immediately, and a
// cached HTML page would happily keep serving a project after revocation.
...
/** Dates are stored as calendar days; render them in UTC so they don't drift. */
...
/** A section heading, consistent across the page. */
...
/** The lifecycle rail. Shows position without ever showing the firm's status. */
  ...
  return (
    <ol className="flex flex-wrap gap-x-1 gap-y-2 sm:flex-nowrap">
      {GATE_SEQUENCE.map((g, i) => {
        ...
        return (
          <li key={g} className="flex min-w-0 flex-1 basis-[45%] flex-col gap-1.5 sm:basis-0">
            <span
              aria-hidden
...
export default async function ClientLinkPage({
  ...
  // without ever confirming a link existed.
  ...
  // Telemetry only, and it cannot throw.
  await recordVisit(link.id);

  return <ClientProjectPage view={view} />;
}

function ClientProjectPage({ view }: { view: ClientProjectView }) {

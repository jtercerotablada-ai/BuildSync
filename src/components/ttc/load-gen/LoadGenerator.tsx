...
// ASCE 7-16/7-22 have NO wind importance factor Iw (removed since 7-10). Risk enters via the
// velocity map (different MRI per Risk Category) — reflected as an approximate V multiplier.
const RISK_VMULT: Record<RiskCategory, string> = { I: '0.94×V', II: '1.00×V', III: '1.06×V', IV: '1.06×V' };
...
function WindTab({ input, units, onSite, onStruct, onSeismic, result, onGoCombos, onExportCsv, onPrint, onSaveJson, onCopyLink }: {
  ...
  // Pressure display unit — adds Tn/m² and kgf/m² (metric-technical) on top of the system unit (Pa / psf).
  const [presU, setPresU] = useState<'sys' | 'knm2' | 'kgfm2' | 'tnm2'>('sys');
  const PRESU: Record<string, { label: string; fromPa: (pa: number) => number; dec: number }> = {
    sys: { label: unitLabel('pressureSmall', units), fromPa: (pa) => fromSI(pa, 'pressureSmall', units), dec: units === 'imperial' ? 1 : 0 },
    knm2: { label: 'kN/m²', fromPa: (pa) => pa * 0.001, dec: 3 },
    kgfm2: { label: 'kgf/m²', fromPa: (pa) => pa * 0.101971621, dec: 1 },
    tnm2: { label: 'Tn/m²', fromPa: (pa) => pa * 0.000101971621, dec: 4 },
  };
  const pu = PRESU[presU].label;
  const disp = (pa: number) => PRESU[presU].fromPa(pa);
  const pv = (pa: number) => disp(pa).toFixed(PRESU[presU].dec);
  const fp = (pa: number) => `${pv(pa)} ${pu}`;
    ...
    <div className="stl-grid">
      ...
      <aside className="stl-inputs lg-sidebar">
        ...
        <Step n={3} title="Enclosure & Factors" icon={<ShieldCheck size={14} />}>
          ...
          <div className="lg-factors">
            <div className="lg-factor"><span>Risk → wind speed</span><strong>{RISK_VMULT[site.riskCategory]} <em>Cat. {site.riskCategory} · §26.5</em></strong></div>
      ...
      <div className="lg-results">
        ...
        <div className="lg-tiles">
          ...
          <Tile k="Governing pressure" icon={<Gauge size={14} />} gov v={derived ? pv(derived.governing.net) : '—'} unit={derived ? pu : undefined}
        ...
        {derived && (
          <>
            {/* MWFRS summary */}
            <div className="lg-tiles">
              <Tile k="qz (at mean roof ht)" v={pv(derived.qh)} unit={pu} tip="Velocity pressure at mean roof height, Eq 26.10-1" />
              ...
              <Tile k="Net pressure range" v={`${pv(derived.netMin)} to ${pv(derived.netMax)}`} unit={pu} tip="Min → max net design pressure" />
            </div>
            ...
            <div className="stl-panel">
              <h4 className="stl-cardh">Pressure Summary
                <select value={presU} onChange={(e) => setPresU(e.target.value as 'sys' | 'knm2' | 'kgfm2' | 'tnm2')} title="Output pressure unit"
                  style={{ marginLeft: '0.5rem', fontSize: '0.72rem', fontWeight: 600, padding: '0.12rem 0.4rem', borderRadius: 6, border: '1px solid var(--lux-line-soft)', background: 'transparent', color: 'inherit', cursor: 'pointer' }}>
                  <option value="sys">{unitLabel('pressureSmall', units)}</option>
                  <option value="knm2">kN/m²</option>
                  <option value="kgfm2">kgf/m²</option>
                  <option value="tnm2">Tn/m²</option>
                </select>
              </h4>

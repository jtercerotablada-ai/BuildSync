...
import { autoDesignContinuous } from '@/lib/rc/autoDesignContinuous';
import type { RebarLayer, ShearZoneInput } from '@/lib/rc/types';
import {
  type BeamInput,
  type BeamAnalysis,
...
function InputsTab({ model, dispatch, engine, demand, setDemand }: {
  ...
  return (
    <div className="slab-inputs-grid">
      ...
      {/* Phase 6 — Continuous beam auto-design + rebar layers table */}
      <ContinuousAutoDesignCard model={model} demand={demand} dispatch={dispatch} />
      <RebarLayersCard model={model} />
...
// ============================================================================
// ============================================================================
// REBAR LAYERS + ZONES TABLE — Phase 6 (SkyCiv-style)
// ============================================================================
//
// Shows the layered rebar layout for continuous beams. Each row in the
// "Longitudinal Rebar Layers" table is a RebarLayer (top/bottom × range).
// Each row in the "Shear Rebar Zones" table is a ShearZoneInput (legs +
// spacing × range). Refer to the SkyCiv "Rebar Layers and Zones" UI:
//   https://skyciv.com/docs/skyciv-rc-design/general/beam-column-standalone/
//
function RebarLayersCard({ model }: { model: BeamInput }) {
  const layers = model.reinforcement.layers;
  const zones = model.reinforcement.shearZones;
  if ((!layers || layers.length === 0) && (!zones || zones.length === 0)) return null;
  return (
    <div className="slab-card" style={{ borderColor: 'rgba(95,182,116,0.35)' }}>
      <h4>📋 Rebar Layers + Zones (SkyCiv-style)</h4>
      <p className="ab-empty" style={{ marginBottom: '0.6rem' }}>
        Layered reinforcement layout — each layer is a top/bottom rebar group running
        from xStart to xEnd. Generated by Auto-Design for continuous beams (matches the
        bending-moment diagram: top bars over supports, bottom bars in midspans, with
        curtailment per ACI §9.7.3).
      </p>
      {layers && layers.length > 0 && (
        <div className="ab-table-scroll" style={{ marginBottom: '0.8rem' }}>
          <h5 style={{ marginBottom: '0.4rem' }}>Longitudinal Rebar Layers</h5>
          <table className="ab-result-table" style={{ fontSize: '0.85rem' }}>
            ...
            <tbody>
              {layers.map((L: RebarLayer, i: number) => (
                <tr key={i}>
                  <td><strong>{L.mark ?? `L${i + 1}`}</strong></td>
                  <td><span style={{
                    color: L.position === 'top' ? '#ff8a72' : '#76b6c9',
                    fontWeight: 600,
                  }}>{L.position}</span></td>
                  <td>{L.count}</td>
                  <td>{L.bar}</td>
                  <td>{L.topBotDistance.toFixed(0)}</td>
                  <td>{L.xStart.toFixed(0)}</td>
                  <td>{L.xEnd.toFixed(0)}</td>
                  <td>{(L.xEnd - L.xStart).toFixed(0)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {zones && zones.length > 0 && (
        <div className="ab-table-scroll">
          <h5 style={{ marginBottom: '0.4rem' }}>Shear Rebar Zones</h5>
          <table className="ab-result-table" style={{ fontSize: '0.85rem' }}>
            <thead>
              <tr>
                <th>Zone</th>
            ...
            <tbody>
              {zones.map((z: ShearZoneInput, i: number) => {
                const len = z.xEnd - z.xStart;
                const count = Math.max(2, Math.floor(len / z.spacing) + 1);
                return (
                  <tr key={i}>
                    <td>S{i + 1}</td>
                    <td>{z.bar}</td>
                    <td>{z.legs}</td>
                    <td>{z.spacing}</td>
                    <td>{z.xStart.toFixed(0)}</td>
                    <td>{z.xEnd.toFixed(0)}</td>
                    <td>{count}</td>
                  </tr>
                );
              })}
...
// ============================================================================
// CONTINUOUS-BEAM AUTO-DESIGN CARD — Phase 6
// ============================================================================
function ContinuousAutoDesignCard({ model, demand, dispatch }: {
  model: BeamInput;
  demand: DemandSource;
  dispatch: React.Dispatch<Action>;
}) {
  const [recommendation, setRecommendation] = useState<ReturnType<typeof autoDesignContinuous> | null>(null);

  // Only show this card if user is in continuous-beam demand mode
  if (demand.kind !== 'continuous') return null;

  const handleDesign = () => {
    const rec = autoDesignContinuous({
      geometry: model.geometry,
      materials: model.materials,
      model: demand.model,
    });
  ...
  return (
    <div className="slab-card" style={{ borderColor: 'rgba(95,182,116,0.55)' }}>
      <h4>🏗️ Auto-Design Continuous Beam (Phase 6)</h4>
      <p className="ab-empty" style={{ marginBottom: '0.6rem' }}>
        Generates a SkyCiv-style layered reinforcement layout from the
        moment envelope:
        <strong> top bars</strong> over supports (negative-moment regions) +
        <strong> bottom bars</strong> in midspans (positive-moment regions),
        with curtailment per ACI §9.7.3 + §9.7.7. Reads continuous-beam
        demand source — switch to &quot;Continuous beam&quot; mode first.
      </p>
      <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.6rem' }}>
        <button type="button" className="ab-btn ab-btn--primary" onClick={handleDesign}>
          🏗️ Run Continuous Auto-Design
        </button>
        {recommendation && (
          <button type="button" className="ab-btn" onClick={handleApply}>
            ✓ Apply Layered Reinforcement
          </button>
        )}
      </div>
      {recommendation && (
        <div className="ab-table-scroll">
          <h5 style={{ marginBottom: '0.4rem' }}>Per-region demand</h5>
          <table className="ab-result-table" style={{ fontSize: '0.85rem', marginBottom: '0.8rem' }}>
            <thead>
              <tr>
                <th>Region</th>
                <th>x_start (mm)</th>
            ...
            <tbody>
              {recommendation.regions.map((r, i) => (
                <tr key={i}>
                  <td>
                    <span style={{
                      color: r.kind === 'positive' ? '#76b6c9' : '#ff8a72',
                      fontWeight: 600,
                    }}>{r.kind === 'positive' ? '+M (bottom)' : '−M (top)'}</span>
                  </td>
                  <td>{r.xStart.toFixed(0)}</td>
                  <td>{r.xEnd.toFixed(0)}</td>
          ...
          <h5 style={{ marginBottom: '0.4rem' }}>Generated layers</h5>
          <table className="ab-result-table" style={{ fontSize: '0.85rem' }}>
            <thead>
              <tr><th>Mark</th><th>Position</th><th>Count·Bar</th><th>x_start</th><th>x_end</th></tr>
            </thead>
            <tbody>
              {recommendation.layers.map((L, i) => (
                <tr key={i}>
                  <td><strong>{L.mark}</strong></td>
                  <td><span style={{
                    color: L.position === 'top' ? '#ff8a72' : '#76b6c9',
                    fontWeight: 600,
                  }}>{L.position}</span></td>
